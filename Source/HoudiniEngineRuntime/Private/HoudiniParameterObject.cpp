...
bool
FHoudiniParameterObjectSortPredicate::operator()(
    const FHoudiniParameterObject & A,
...
FHoudiniParameterObject::HapiGetParmInfo( HAPI_ParmInfo & ParmInfo ) const
{
    ...
    if ( FHoudiniApi::GetParameters(
        FHoudiniEngine::Get().GetSession(), NodeId, &ParmInfo,
        ParmId, 1 ) != HAPI_RESULT_SUCCESS )
    {
        return false;
    }
...
FHoudiniParameterObject::HapiGetValues( TArray< float > & Values ) const
{
    ...
    if ( ParmInfo.size > 0 )
    {
        ...
        if ( FHoudiniApi::GetParmFloatValues(
            ...
            &Values[ 0 ], ParmInfo.floatValuesIndex,
            ParmInfo.size ) != HAPI_RESULT_SUCCESS )
        {
            return false;
        }
    }
...
FHoudiniParameterObject::HapiGetValues( TArray< FHoudiniEngineString > & Values ) const
{
    ...
    if ( ParmInfo.size > 0 )
    {
        ...
        if ( FHoudiniApi::GetParmStringValues(
            FHoudiniEngine::Get().GetSession(), NodeId, false,
            &StringHandles[ 0 ], ParmInfo.stringValuesIndex,
            ParmInfo.size ) != HAPI_RESULT_SUCCESS )
        {
            return false;
        }
...
FHoudiniParameterObject::HapiSetValue( const FString & Value ) const
{
    ...
    if ( FHoudiniApi::SetParmStringValue(
        FHoudiniEngine::Get().GetSession(), NodeId,
        StringValue.c_str(), ParmInfo.id, 0 ) != HAPI_RESULT_SUCCESS )
    {
        return false;
    }
...
FHoudiniParameterObject::HapiSetValues( const TArray< int32 > & Values ) const
{
    ...
    if ( FHoudiniApi::SetParmIntValues(
        FHoudiniEngine::Get().GetSession(), NodeId, &Values[0],
        ParmInfo.intValuesIndex, Values.Num() ) != HAPI_RESULT_SUCCESS )
    {
        return false;
    }
...
FHoudiniParameterObject::HapiSetValues( const TArray< float > & Values ) const
{
    ...
    if ( FHoudiniApi::SetParmFloatValues(
        FHoudiniEngine::Get().GetSession(), NodeId, &Values[ 0 ],
        ParmInfo.floatValuesIndex, Values.Num() ) != HAPI_RESULT_SUCCESS )
    {
        return false;
    }
