	...
	HAPI_GroupType GroupType, TArray<FString>& GroupNames)
{
	...
	if(GroupCount > 0)
	{
		...
		HOUDINI_CHECK_ERROR_RETURN(FHoudiniApi::GetGroupNames(FHoudiniEngine::Get().GetSession(), AssetId, ObjectId,
...
void
FHoudiniEngineUtils::HapiRetrieveParameterNames(const TArray<HAPI_ParmInfo>& ParmInfos, TArray<std::string>& Names)
{
	...
	int32 TupleSize)
{
	...
	HOUDINI_CHECK_ERROR_RETURN(FHoudiniApi::GetAttributeFloatData(FHoudiniEngine::Get().GetSession(), AssetId,
		ObjectId, GeoId, PartId, Name, &AttributeInfo, &Data[0], 0, AttributeInfo.count), false);
			...
			if(LightMap2D)
			{
				if(LightMap2D->IsValid(0))
				{
					...
					if(TextureLightmap)
					{
						...
						if(!LightmapMipSizeX || !LightmapMipSizeY)
						{
							LightmapMipData.Empty();
						}
					}
