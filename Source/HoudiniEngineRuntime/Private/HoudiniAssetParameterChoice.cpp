	...
	UHoudiniAssetParameter* InParentParameter, HAPI_NodeId InNodeId, const HAPI_ParmInfo& ParmInfo)
{
	...
	if(HAPI_PARMTYPE_INT == ParmInfo.type)
	{
		...
		if(HAPI_RESULT_SUCCESS != FHoudiniApi::GetParmIntValues(FHoudiniEngine::Get().GetSession(), NodeId,
			&CurrentValue, ValuesIndex, TupleSize))
		{
			return false;
		}
	}
	else if(HAPI_PARMTYPE_STRING == ParmInfo.type)
	{
		...
		if(HAPI_RESULT_SUCCESS != FHoudiniApi::GetParmStringValues(FHoudiniEngine::Get().GetSession(), NodeId,
			false, &StringHandle, ValuesIndex, TupleSize))
		{
			return false;
		}
	...
	if(HAPI_RESULT_SUCCESS != FHoudiniApi::GetParmChoiceLists(FHoudiniEngine::Get().GetSession(), NodeId,
		&ParmChoices[0], ParmInfo.choiceIndex, ParmInfo.choiceCount))
	{
		return false;
	}
...
UHoudiniAssetParameterChoice::UploadParameterValue()
{
	...
	else
	{
		...
		FHoudiniApi::SetParmIntValues(FHoudiniEngine::Get().GetSession(), NodeId, &CurrentValue, ValuesIndex,
			TupleSize);
	}
