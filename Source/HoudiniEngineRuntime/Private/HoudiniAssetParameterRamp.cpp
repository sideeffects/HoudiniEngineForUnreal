...
UHoudiniAssetParameterRamp::OnCurveEditingFinished()
{
	if(bIsCurveChanged)
	{
		...
		if(HoudiniAssetParameterRampCurveFloat)
		{
			...
			TArray<FRichCurveKey> RichCurveKeysCopy = RichCurve.GetCopyOfKeys();
		...
		{
			TArray<FRichCurveKey> RichCurveKeysCopyR =
				HoudiniAssetParameterRampCurveColor->FloatCurves[0].GetCopyOfKeys();
			TArray<FRichCurveKey> RichCurveKeysCopyG =
				HoudiniAssetParameterRampCurveColor->FloatCurves[1].GetCopyOfKeys();
			TArray<FRichCurveKey> RichCurveKeysCopyB =
				HoudiniAssetParameterRampCurveColor->FloatCurves[2].GetCopyOfKeys();
			TArray<FRichCurveKey> RichCurveKeysCopyA =
				HoudiniAssetParameterRampCurveColor->FloatCurves[3].GetCopyOfKeys();

			MarkPreChanged();

			// We need to update ramp key positions.
			for(int32 KeyIdx = 0, KeyNum = RichCurveKeysCopyR.Num(); KeyIdx < KeyNum; ++KeyIdx)
			{
				UHoudiniAssetParameterFloat* ChildParamPosition = nullptr;
				...
				const FRichCurveKey& RichCurveKeyR = RichCurveKeysCopyR[KeyIdx];
				const FRichCurveKey& RichCurveKeyG = RichCurveKeysCopyG[KeyIdx];
				const FRichCurveKey& RichCurveKeyB = RichCurveKeysCopyB[KeyIdx];
				//const FRichCurveKey& RichCurveKeyA = RichCurveKeysCopyA[KeyIdx];
